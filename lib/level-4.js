...
export function canMakeAmount(target, drawer) {
  ...
  //.floor 
  //Math.floor - rounds 
  //maths bit .Math.min (smallest value)
  let remainder = target

  for (let i = 0; i < drawer.length; i++) {
    const denomination = drawer [i]

    const amountNeeded = Math.floor (remainder / denomination.value)

    remainder -= denomination.value * Math.min(amountNeeded, denomination.quantity)
    if (remainder ===0) {
      return true
    }
  }
